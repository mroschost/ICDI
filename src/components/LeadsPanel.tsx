          ...
          {filteredLeads.length === 0 ? (
            ...
            filteredLeads.map((lead) => {
              ...
              <motion.div 
                layout
                key={leadId}
                className={`border rounded-2xl transition-all ${expandedLead === leadId ? 'border-gov-blue-200 bg-gov-blue-50/30' : 'border-slate-100 bg-white hover:border-gov-blue-100'}`}
              >
                <div 
                  className="p-4 flex items-center justify-between cursor-pointer"
                  onClick={() => setExpandedLead(expandedLead === leadId ? null : leadId)}
                >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {expandedLead === leadId ? <ChevronUp className="w-5 h-5 text-slate-300" /> : <ChevronDown className="w-5 h-5 text-slate-300" />}
                  </div>
                </div>
